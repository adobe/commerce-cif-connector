...
public class CommerceTestBase {
  ...
  public static final String GRAPHQL_CLIENT_FACTORY_PID =
  ...
  private static final String VIRTUAL_CATALOG_BUNDLE_NAME =
      "com.adobe.commerce.cif.virtual-catalog";
  ...
  public static void initCommerceClient()
    ...
    cAuthorAuthor =
        cqBaseClassRule.authorRule.getClient(CommerceClient.class, "imccoy", "password");

    String httpsPort = String.valueOf(mockServerRule.getHttpsPort());

    // This configures the lower-level GraphQL client. The Magento client is configured in the
    // test-content package.
    graphqlOsgiConfig.withIdentifier("default")
        .withUrl("https://localhost:" + httpsPort + "/graphql")
        .withAcceptSelfSignedCertificates(true).withCatalogCachingSchedulerEnabled(false);

    updateOSGiConfiguration(cAdminAuthor, graphqlOsgiConfig.build(), GRAPHQL_CLIENT_BUNDLE,
        GRAPHQL_CLIENT_FACTORY_PID);
  }
   ...
   */
  private static String getConfigurationPid(OsgiConsoleClient osgiClient, String factoryPID)
      throws ClientException {
  ...
  protected static void updateOSGiConfiguration(CommerceClient client, Map<String, Object> config,
      String bundle, String factoryPID)
  ...
  private static void restartVirtualCatalogBundle(OsgiConsoleClient osgiConsoleClient)
      throws ClientException, TimeoutException, InterruptedException {
