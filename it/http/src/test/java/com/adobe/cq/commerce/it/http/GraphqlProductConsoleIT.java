...
public class GraphqlProductConsoleIT extends CommerceTestBase {
  ...
  private static final String CORAL_COLUMN_FORMAT_EQUALS =
      "coral-columnview-item[data-foundation-collection-item-id=%s]";
  private static final String CORAL_COLUMN_FORMAT_STARTS_WITH =
      "coral-columnview-item[data-foundation-collection-item-id^=%s]";
  ...
  private static final String FOLDER_PROPERTIES =
      "/mnt/overlay/commerce/gui/content/products/folderproperties.html";

  @BeforeClass
  public static void setup() throws ClientException, InterruptedException {
    // Create product binding
    HttpEntity params = FormEntityBuilder.create().addParameter("_charset_", "utf-8")
        .addParameter("./jcr:title", "graphql").addParameter(":name", "graphql")
        .addParameter("./cq:catalogDataResourceProviderFactory", "magento-graphql")
        .addParameter("./cq:catalogIdentifier", "default").addParameter("./jcr:language", "en_us")
        .addParameter("./jcr:primaryType", "sling:Folder").build();
  ...
  public static RequestResponseRule.Builder CATALOG_RULE = rule()
      .on(request().withMethod(HttpMethod.POST).withRequestURI("/graphql")
          .withBody(s -> s.startsWith("{\"query\":\"{category(id:4)")))
      .send(response().withStatus(HttpStatus.OK_200)
          .withContentFromResource(
              "/com/adobe/cq/commerce/it/http/magento-graphql-category-tree-2.3.1.json")
  ...
  public static RequestResponseRule.Builder SEARCH_PRODUCTS_IN_CATEGORY = rule()
      ...
      .send(response().withStatus(HttpStatus.OK_200)
          .withContentFromResource(
              "/com/adobe/cq/commerce/it/http/magento-graphql-category-products.json")
          .withContentType("application/json; charset=utf-8"));

  public static RequestResponseRule.Builder SEARCH_PRODUCT_BY_SKU = rule()
      .on(request().withMethod(HttpMethod.POST).withRequestURI("/graphql").withBody(
          s -> s.startsWith("{\"query\":\"{products(filter:{sku:{eq:\\\"meskwielt\\\"}})")))
      .send(response().withStatus(HttpStatus.OK_200)
  ...
  public void testCategoryRoot() throws Exception {
    ...
    // Perform
    SlingHttpResponse response = cAuthorAuthor.doGet(
        "/libs/commerce/gui/content/products.html" + JCR_BASE_PATH, null, NO_CACHE_HEADERS, SC_OK);
    ...
    Assert.assertTrue(
        doc.select(String.format(CORAL_COLUMN_FORMAT_EQUALS, JCR_BASE_PATH + "/equipment"))
            .size() > 0);
    Assert.assertTrue(
        doc.select(String.format(CORAL_COLUMN_FORMAT_EQUALS, JCR_BASE_PATH + "/men")).size() > 0);
    Assert.assertTrue(
        doc.select(String.format(CORAL_COLUMN_FORMAT_EQUALS, JCR_BASE_PATH + "/women")).size() > 0);

    // Check that child categories are not displayed
    Assert.assertTrue(
        doc.select(String.format(CORAL_COLUMN_FORMAT_EQUALS, JCR_BASE_PATH + "/men/pants"))
            .size() == 0);
  }
  ...
  public void testProductsInCategory() throws Exception {
    ...
    // Perform
    SlingHttpResponse response = cAuthorAuthor
        .doGet("/aem/products.html" + JCR_BASE_PATH + "/men/coats", null, NO_CACHE_HEADERS, SC_OK);
    ...
    Assert.assertTrue(doc
        .select(String.format(CORAL_COLUMN_FORMAT_EQUALS, JCR_BASE_PATH + "/men/coats/meotwibrt"))
        .size() > 0);

    // Check that products from other categories are not displayed
    Assert.assertTrue(doc
        .select(
            String.format(CORAL_COLUMN_FORMAT_EQUALS, JCR_BASE_PATH + "/men/footwear/meotwisus"))
        .size() == 0);
  }
  ...
  public void testProductDetails() throws Exception {
    ...
    SlingHttpResponse response =
        cAuthorAuthor.doGet("/aem/products.html" + JCR_BASE_PATH + "/men/coats/meskwielt", null,
    ...
    // Check variants
    Assert.assertEquals(15, doc.select(String.format(CORAL_COLUMN_FORMAT_STARTS_WITH,
        JCR_BASE_PATH + "/men/coats/meskwielt/meskwielt-")).size());
  }
  ...
  public void testCcifFolderProperties() throws Exception {
    mockServerRule.add(CATALOG_RULE.build());
    SlingHttpResponse response =
        cAuthorAuthor.doGet(FOLDER_PROPERTIES + JCR_BASE_PATH + "/men", SC_OK);
  ...
  public void testJCRFolderProperties() throws Exception {
    mockServerRule.add(CATALOG_RULE.build());
    SlingHttpResponse response = cAuthorAuthor
        .doGet(FOLDER_PROPERTIES + "/var/commerce/products/" + JCR_PRODUCT_ROOT, SC_OK);
  ...
  public void testCategoryFolderProperties() throws Exception {
    ...
    // Perform
    SlingHttpResponse response = cAuthorAuthor.doGet(
        FOLDER_PROPERTIES + JCR_BASE_PATH + "/women/footwear", null, NO_CACHE_HEADERS, SC_OK);
