...
public class CatalogDataResourceProviderManagerImplTest extends RepositoryBaseTest {
  ...
  public void beforeTest() throws Exception {
    ...
    Mockito
        .when(bundleContext.registerService((Class) Mockito.any(), (ResourceProvider) Mockito.any(),
            Mockito.any()))
        .thenAnswer(
  ...
  private void testBindFactoryCreateRemoveRoots(int createCount, int removeCount) throws Exception {
    if (removeCount > createCount) {
      throw new IllegalArgumentException(
          "removeCount above createCount: " + removeCount + " > " + createCount);
    }
  ...
  private Map<String, CatalogDataResourceProviderFactory<?>> getProviders()
      throws NoSuchFieldException {
    return (Map<String, CatalogDataResourceProviderFactory<?>>) PrivateAccessor.getField(manager,
        "providers");
  }

  private Map<ResourceProvider, ServiceRegistration<?>> getProviderRegistrations()
      throws NoSuchFieldException {
    return (Map<ResourceProvider, ServiceRegistration<?>>) PrivateAccessor.getField(manager,
  ...
  private class FactoryConfig {
    ...
    FactoryConfig() {
      CatalogDataResourceProviderFactory providerFactory =
          Mockito.mock(CatalogDataResourceProviderFactory.class);
      Mockito.when(providerFactory.createResourceProvider(Mockito.any())).thenAnswer(
          (Answer<ResourceProvider>) invocation -> Mockito.mock(ResourceProvider.class));
