...
public class MagentoProduct implements Product {
   ...
   * All the methods that access properties (e.g. prices, descriptions, etc) defined in both the
   * active variant and the base product will first get the properties defined in the variant and
   * will fallback to the property of the base product if a variant property is null.
   * 
   * @param resourceResolver The resource resolver for that resource.
   * @param path The resource path of this product.
   * @param product The Magento base product.
   * @param activeVariantSku The SKU of the "active" variant product or null if this product
   *        represents the base product.
   */
  public MagentoProduct(ResourceResolver resourceResolver, String path, ProductInterface product,
    ...
    if (product instanceof ConfigurableProduct) {
      ...
      if (cp.getVariants() != null && cp.getVariants().size() > 0) {
        if (activeVariantSku != null) {
          masterVariant = cp.getVariants().stream().map(cv -> cv.getProduct())
              .filter(sp -> activeVariantSku.equals(sp.getSku())).findFirst().orElse(null);
        }
  ...
  private String getVariantOrBaseProperty(Supplier<String> productProvider,
      Supplier<String> masterVariantProvider) {
  ...
  public String getSKU() {
    return getVariantOrBaseProperty(product::getSku,
        masterVariant != null ? masterVariant::getSku : null);
  }
  ...
  public String getTitle(String selectorString) {
    return getVariantOrBaseProperty(product::getName,
        masterVariant != null ? masterVariant::getName : null);
  }
  ...
  public String getDescription(String selectorString) {
    return getVariantOrBaseProperty(() -> product.getDescription().getHtml(),
        masterVariant != null ? (() -> masterVariant.getDescription().getHtml()) : null);
  }

  @Override
  public String getThumbnailUrl() {
    return getVariantOrBaseProperty(() -> product.getThumbnail().getUrl(),
  ...
  public String getImageUrl() {
    return getVariantOrBaseProperty(() -> product.getImage().getUrl(),
  ...
  public ImageResource getImage() {
    String url = getImageUrl();
    return new SyntheticImageResource(resourceResolver, path + "/image",
        SyntheticImageResource.IMAGE_RESOURCE_TYPE, url);
  }
