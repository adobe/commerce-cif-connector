...
class ResourceMapper<T> {
   ...
   * <ul>
   * <li>to initially build the cache in case the Sling scheduler has not yet started the
   * <code>cacheRefreshJob</code></li>
   * <li>to build the "cache" for each method call in case caching is disabled (in which case the
   * "cache" is of no use, except to build all the category paths)</li>
   * </ul>
   */
  private void init() {
    if (!initDone) {
  ...
  /**
   * This method builds various category caches used to lookup categories and products in a faster
   * way.
   */
  private void buildAllCategoryPaths() {
    ...
    while (it.hasNext()) {
      ...
      if (StringUtils.isAnyBlank(child.getName(), child.getUrlPath())) {
        LOGGER.warn("Ignoring top-level category " + child.getId()
            + " with null or empty url path and/or name");
        it.remove();
      }
    }
  ...
  private void buildAllCategoryPathsFor(CategoryTree categoryTree,
      Map<String, CategoryTree> categoryByPathsMap, Map<Integer, String> categoryPathsByIdMap) {

    LOGGER.debug(
        "Adding cached category " + categoryTree.getId() + " --> " + categoryTree.getUrlPath());
    ...
    while (it.hasNext()) {
      ...
      if (StringUtils.isAnyBlank(child.getName(), child.getUrlPath())) {
        LOGGER.warn(
            "Ignoring category " + child.getId() + " with null or empty url path and/or name");
        it.remove();
      }
      buildAllCategoryPathsFor(child, categoryByPathsMap, categoryPathsByIdMap);
    }
  ...
  CategoryResource resolveCategory(ResolveContext<T> ctx, String path) {
    ...
    // Remove root (/var/commerce/products/cloudcommerce) then try to find the category path
    // /Men/Coats
  ...
  ProductResource resolveProduct(ResolveContext<T> ctx, String path) {
    ...
    try {
      ...
      if (product != null && product.getId() != null) {
        ...
        return new ProductResource(ctx.getResourceResolver(), path, product,
            isVariant ? productParts.get(1) : null);
      }
    } catch (Exception e) {
  ...
  SyntheticImageResource resolveProductImage(ResolveContext<T> ctx, String path) {
    ...
    try {
      ...
      if (product != null) {
        ...
        if (imageUrl != null) {
          return new SyntheticImageResource(ctx.getResourceResolver(), path,
  ...
  private List<String> resolveProductParts(String path) {
    ...
    // Remove root (/var/commerce/products/cloudcommerce) then try to find category
    // /Men/Coats/meskwielt.1-s
    // --> that category is not found, so we try to find the category /Men/Coats
    // --> we find the category /Men/Coats and try to fetch the product meskwielt.1-s and variant
    // meskwielt.2-l
    ...
    Set<String> categoryPaths = categoryByPaths.keySet(); // defensive copy in case the cache is
                                                          // updated while looping
    ...
    if (children.isEmpty()) {
      try {
        List<ProductInterface> products =
            graphqlDataService.getCategoryProducts(Integer.valueOf(parentCifId));
        ...
        LOGGER.error(
            "Error while fetching category products for " + parentPath + " (" + parentCifId + ")",
            e);
      }
    }
      ...
      if (productInterface != null && productInterface instanceof ConfigurableProduct) {
        ...
        if (variants != null && !variants.isEmpty()) {
          ...
          for (ConfigurableVariant variant : variants) {
            ...
            children
                .add(new ProductResource(resolver, path, simpleProduct, simpleProduct.getSku()));
          ...
          if (imageUrl != null) {
            ...
            Resource imageResource = new SyntheticImageResource(ctx.getResourceResolver(),
                imagePath, SyntheticImageResource.IMAGE_RESOURCE_TYPE, imageUrl);
