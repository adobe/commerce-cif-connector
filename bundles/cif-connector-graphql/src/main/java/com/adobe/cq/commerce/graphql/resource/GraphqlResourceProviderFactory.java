 ...
 */
@Component(service = {CatalogDataResourceProviderFactory.class, CatalogIdentifier.class},
    immediate = true, property = {CatalogDataResourceProviderFactory.PROPERTY_FACTORY_SERVICE_ID
        + "=" + MAGENTO_GRAPHQL_PROVIDER})
public class GraphqlResourceProviderFactory<T>
    implements CatalogDataResourceProviderFactory<T>, CatalogIdentifier {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(GraphqlResourceProviderFactory.class);

  protected Map<String, GraphqlDataService> clients = new ConcurrentHashMap<>();

  @Reference(service = GraphqlDataService.class, bind = "bindGraphqlDataService",
      unbind = "unbindGraphqlDataService", cardinality = ReferenceCardinality.AT_LEAST_ONE,
  ...
  public ResourceProvider<T> createResourceProvider(Resource root) {
    ...
    String catalogIdentifier = properties
        .getInherited(GraphqlDataServiceConfiguration.CQ_CATALOG_IDENTIFIER, String.class);
    if (catalogIdentifier == null) {
      LOGGER.warn(
          "Could not find cq:catalogIdentifier property for given resource at " + root.getPath());
      return null;
    }
    ...
    if (client == null) {
      LOGGER.warn(
          "No MagentoGraphqlClient instance available for catalog identifier " + catalogIdentifier);
      return null;
    }

    ResourceProvider<T> resourceProvider =
        new GraphqlResourceProvider<T>(root.getPath(), client, scheduler);
