...
class GraphqlQueryLanguageProvider<T> implements QueryLanguageProvider<T> {
  ...
  GraphqlQueryLanguageProvider(ResourceMapper<T> resourceMapper,
  ...
  public Iterator<Resource> findResources(ResolveContext<T> ctx, String query, String language) {
    ...
    Integer offset = NumberUtils.createInteger((queryParameters.get(OFFSET_PARAMETER) == null) ? ""
        : queryParameters.get(OFFSET_PARAMETER).toString());
    Integer limit = NumberUtils.createInteger((queryParameters.get(LIMIT_PARAMETER) == null) ? ""
        : queryParameters.get(LIMIT_PARAMETER).toString());
    ...
    List<ProductInterface> products =
        graphqlDataService.searchProducts(fulltext, currentPage, limit);
    ...
    for (ProductInterface product : products) {
      ...
      if (categories != null && !categories.isEmpty()) {
        for (CategoryInterface category : categories) {
          String path =
              resourceMapper.getRoot() + "/" + category.getUrlPath() + "/" + product.getSku();
  ...
  @Override
  public Iterator<ValueMap> queryResources(ResolveContext<T> paramResolveContext, String query,
      String language) {
    return null;
  }
