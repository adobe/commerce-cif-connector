...
class GraphqlResourceProvider<T> extends ResourceProvider<T> {
  ...
  @Override
  public Resource getResource(ResolveContext<T> ctx, String path, ResourceContext resourceContext,
      Resource parent) {
    ...
    if (path.contains(JcrConstants.JCR_CONTENT) || path.endsWith(".jpg") || path.contains("assets")
        || path.contains("thumbnail") || path.contains("rep:policy")) {
      return null;
    }
